...
func (section *RSection) clean() {
	...
	if section.OpenStatus {
		section.status = OPEN.String()
	} else {
		section.status = CLOSED.String()
	}
...
func (section RSection) instructor() (instructors []*Instructor) {
	for _, instructor := range section.Instructor {
		instructors = append(instructors, &Instructor{Name: instructor.Name})
	}
	return
}

func (section RSection) metadata() (metadata []*Metadata) {
...
func (course RCourse) metadata() (metadata []*Metadata) {
